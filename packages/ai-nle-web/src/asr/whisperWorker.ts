...
const WEB_MODEL_ID = "Xenova/whisper-tiny";
...
const initPipeline = async (model: AsrModelSize, language: string) => {
	if (model !== "tiny") {
		throw new Error("Web 仅支持 Tiny 模型");
	}
	if (!asrPipeline || model !== currentModel) {
		asrPipeline = await pipeline("automatic-speech-recognition", WEB_MODEL_ID);
		currentModel = model;
	}
...
self.onmessage = async (event: MessageEvent<WhisperWorkerMessage>) => {
	...
	try {
		...
		if (data.type === "transcribe") {
			if (!asrPipeline) {
				await initPipeline("tiny", currentLanguage || "auto");
			}
