...
import type { PartialExportAudioDspSettings } from "../audio-system/settings";
import { chooseSessionInstructionCandidate } from "../audio-system/sessionInstructionSelector";
import {
	type AudioMixClip,
	type AudioMixInstruction,
	buildTransitionAudioMixPlan,
	type TransitionAudioCurve,
} from "../audio-system/transitionAudioMix";
import type {
...
} from "../timeline-system/transitionFrameState";
import type { TimelineTrack } from "../timeline-system/timeline";
import type { AudioTrackControlStateMap } from "../timeline-system/utils/audioTrackState";
import { resolveTimelineEndFrame } from "../timeline-system/utils/timelineEndFrame";
import { isTimelineTrackAudible } from "../timeline-system/utils/trackAudibility";
import { isVideoSourceAudioMuted } from "../timeline-system/utils/videoSourceAudio";
...
export const exportTimelineAsVideoCore = async (
	...
	let persistentSurface: JsiSkSurface | null = null;
	let persistentSkiaCanvas: ReturnType<JsiSkSurface["getCanvas"]> | null = null;
	...
	try {
		...
		const canvasVideoSource = new CanvasSource(getExportCanvas(), {
			codec: "avc",
			bitrate: QUALITY_HIGH,
		});
		output.addVideoTrack(canvasVideoSource, { frameRate: fps });
		...
		const createPersistentSurface = () => {
			...
			persistentSurface = surfaceResult.surface;
			persistentSkiaCanvas = surfaceResult.surface.getCanvas();
		};
		const createFrameSurface = () => {
			if (renderBackend.kind === "webgpu") {
				...
				const frameSurface = surfaceResult.surface;
				return {
					surface: frameSurface,
					skiaCanvas: frameSurface.getCanvas(),
					dispose: () => {
						frameSurface.dispose();
					},
				};
			}
			if (!persistentSurface || !persistentSkiaCanvas) {
				throw new Error("导出失败：无法创建当前帧 Surface");
			}
			return {
				surface: persistentSurface,
				skiaCanvas: persistentSkiaCanvas,
				dispose: () => {},
			};
		};
		if (renderBackend.kind !== "webgpu") {
			createPersistentSurface();
		}
		...
		for (let frame = startFrame; frame < endFrame; frame += 1) {
			...
			if (useLiveRenderState) {
				...
				try {
					...
					throwIfAborted(options.signal);
					if (!liveRoot) {
						throw new Error("导出失败：缺少 Skia live root");
					}
					const frameSurface = createFrameSurface();
					try {
			...
			try {
				...
				const frameSurface = createFrameSurface();
				try {
					frameSurface.skiaCanvas.drawPicture(picture);
					frameSurface.surface.flush();
				} finally {
					frameSurface.dispose();
				}
